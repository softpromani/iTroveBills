<template>
    ...
    <div class="no-print p-3 bg-light border-bottom sticky-top d-flex align-items-center justify-content-center">
        ...
        <Link :href="route('create.eway.bill', { invoice_id: props.invoice.id, type: 'gst' })" class="btn btn-primary mx-2">
            <i class="fa fa-truck mr-2"></i> Generate E-Way Bill
        </Link>
        <Link :href="route('gst.invoice.list')" class="btn btn-outline-secondary mx-2">
    ...
    <div class="invoice-wrapper py-4 px-2">
        <div class="invoice-container shadow-sm mx-auto bg-white border-2 border-dark">
            ...
            <div class="p-2 border-top border-dark x-small text-uppercase">
                <span class="font-weight-bold">Tax Amount (in words) :</span> <strong>INR {{ taxAmountInWords }} Only</strong>
            </div>
...
// HSN Calculation logic
const getHsnSummary = () => {
    const summary = {};
    props.invoice.invoiceitems?.forEach(item => {
        const hsn = item.hsn_code || 'N/A';
        const rate = parseFloat(item.gst_percentage || 0);
        const taxable = parseFloat(item.rate * item.quantity);
        const tax = (taxable * rate) / 100;

        if (!summary[hsn]) {
            summary[hsn] = { hsn, taxable: 0, tax: 0, rate };
        }
...
onMounted(() => {
    amountInWords.value = getWords(props.invoice.subtotal_amount || props.invoice.total_ammount);
